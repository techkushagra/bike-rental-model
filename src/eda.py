"""
Exploratory Data Analysis for the Yolo Rental Bike problem.
...
def run_eda(output_dir):
    ...
    print("\n--- Demand Statistics ---")
    print(f"Total weekly demand: {demand_df.values.sum()} bike-rentals")
    print(f"Peak demand: {demand_df.values.max()} (HSR_Layout Monday)")
    print(f"Min demand: {demand_df.values.min()} (Marathahalli Wednesday)")
    print(f"Average daily demand per depot: {demand_df.values.mean():.0f}")
    print(f"\nDaily totals:")
    for day in DAYS:
        total = sum(DEMAND[i][day] for i in DEPOTS)
        print(f"  {day}: {total} bikes")
    print(f"\nDepot totals (weekly):")
    for depot in DEPOTS:
        total = sum(DEMAND[depot].values())
        print(f"  {depot}: {total} bikes ({total/demand_df.values.sum()*100:.1f}%)")

    # --- Flow Network Analysis ---
    print("\n--- Return Flow Analysis ---")
    print("Net flow direction (bikes tend to accumulate at):")
    for j in DEPOTS:
        ...
        print(f"  {j:14s}: net flow = {net:+.3f} ({direction})")

    # --- Repair Bottleneck Analysis ---
    print("\n--- Repair Bottleneck Analysis ---")
    total_repair_cap = sum(REPAIR_CAPACITY.values())
    print(f"Total repair capacity: {total_repair_cap} bikes/day")
    max_sustainable = total_repair_cap / DAMAGE_RATE
    print(f"Max sustainable rentals (damage equilibrium): {max_sustainable:.0f} bikes/day")
    avg_daily_demand = demand_df.values.sum() / len(DAYS)
    print(f"Average daily demand: {avg_daily_demand:.0f} bikes/day")
    print(f"Demand vs repair capacity ratio: {avg_daily_demand/max_sustainable:.2f}")
    if avg_daily_demand > max_sustainable:
        print(f"  -> BOTTLENECK: demand exceeds repair throughput by {avg_daily_demand - max_sustainable:.0f} bikes/day")
    ...
    print(f"Weighted avg revenue per rental: INR {weighted_rev:.2f}")
    print(f"Weighted avg marginal cost: INR {weighted_cost:.2f}")
    print(f"Gross margin per rental: INR {weighted_rev - weighted_cost:.2f}")
    print(f"Break-even fleet utilization (to cover INR {OPPORTUNITY_COST}/bike/week):")
    min_rentals_per_bike = OPPORTUNITY_COST / (weighted_rev - weighted_cost)
    print(f"  Need {min_rentals_per_bike:.2f} rentals/bike/week to break even")

    # --- Generate Plots ---
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("Yolo Rental Bike - Exploratory Data Analysis", fontsize=13, fontweight="bold")
    ...
    ax.set_title("Demand Heatmap (bikes/day)")
    ...
    ax = axes[0, 1]
    positions = {"Bellandur": (0.2, 0.8), "HSR_Layout": (0.8, 0.8),
                 "Whitefield": (0.5, 0.4), "Marathahalli": (0.2, 0.1)}
    ...
    ax.set_xticklabels([d[:5] for d in DEPOTS])
    ax.set_yticks(range(len(DEPOTS)))
    ax.set_yticklabels([d[:5] for d in DEPOTS])
    ax.set_title("Transfer Cost Matrix (INR)")
    plt.colorbar(im, ax=ax)
    for i in range(len(DEPOTS)):
